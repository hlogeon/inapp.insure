                    ...
                    <div class="subscribe-wrap">
                            ...
                            <router-link to="/account" class="backto">
                                ...
                                >
                                    <path
                                        fill-rule="evenodd"
                                        clip-rule="evenodd"
                                        d="M21.2147 21.211C23.0172 19.4086 25.8351 19.3043 27.5088 20.9779L63.8743 57.3434C65.5479 59.0171 65.4436 61.835 63.6411 63.6374C61.8387 65.4399 59.0208 65.5442 57.3471 63.8706L20.9816 27.5051C19.308 25.8314 19.4123 23.0135 21.2147 21.211Z"
                                        fill="#D0D0D0"
                                    />
                                    <path
                                        fill-rule="evenodd"
                                        clip-rule="evenodd"
                                        d="M21.2128 63.6386C19.4104 61.8362 19.306 59.0182 20.9797 57.3445L57.3452 20.9791C59.0188 19.3054 61.8368 19.4097 63.6392 21.2122C65.4416 23.0146 65.546 25.8325 63.8723 27.5062L27.5068 63.8717C25.8331 65.5454 23.0152 65.441 21.2128 63.6386Z"
                                        fill="#D0D0D0"
                                    />
                                </svg>
                            </router-link>
                        ...
                        <p class="subscribe__subtext" v-if="tarrif">
                            <template v-if="policy">
                                Следующее списание в размере
                                {{ tarrif.amount }} ₽ произойдет
                                {{ dateFormat(policy.finish) }}
                            </template>
                            <template v-else>
                                Следующее списание в размере
                                {{ tarrif.amount }} ₽
...
export default {
    ...
    methods: {
        updatePage() {
            ...
            axios
                .get("/api/v1/get_tarrifs", { params: data })
                .then(response => {
                    //console.log('tarrifs', response.data)
                    if (
                        response.data.status &&
                        response.data.data.policy.subscribed
                    ) {
        ...
        changeTarrif() {
            ...
            axios
                .get("/api/v1/set_tarrifs", { params: data })
                .then(response => {
                    if (response.data.status) {
                        ...
                        new StatusPopup({
                            text:
                                "Ваш тариф был изменен, новые изменения вступят с " +
                                response.data.data,
                            delay: 2000
                        });
                    } else
                        this.error =
                            "Произошла ошибка во время изменения тарифа";
                });
        },
        dateFormat($date) {
