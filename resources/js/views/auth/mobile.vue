		...
		<section class="step-section">
 		 	<div class="container">
			    ...
			    <ul class="steps-list for-mobile-inline-flex" v-if="prev">
 		 	    ...
 		 	    <h1 class="title">
 		 	    	<a @click="previousPage" v-if="prev" class="button button-arrow button-arrow--prev">
           			  <svg class="svg">
 		 	    ...
 		 	    <div class="step-form">
 		 	      	<div class="step-phone-wrapper">
 		 	      	  <div class="input-wrapper">
 		 	      	    <label class="input-label">
 		 	      	    	<input required class="input phone input--big j_mask" type="tel" v-model="phone" placeholder="Номер телефона" />
 		 	     	...
 		 	     	<button class="button submit-button" type="button" :disabled="disabled" v-on:click="sendPhoneAuth()">
 		 	     	  	Продолжить
 		 	     	</button>
 		 	    </div>
 		 	    <!-- ./ End of form -->
						...
						if(response.data.status) {
							if(response.data.data.phone)
								if(response.data.data.hasOwnProperty('nextStep') && response.data.data.nextStep.length > 0)
									this.$router.push({path: response.data.data.nextStep, query: {previously: this.$route.query.redirect}})
								else {
									this.$router.push({ path: '/authsms', query: this.$route.query })
								}
						} else {
							this.disabled = false
