		...
		<template v-else>
			<div class="header-controls">
			  <div id="profile-dropdown" class="profile dropdown j_dropdown">
			    <div data-dropdown-target="#profile-dropdown" role="button" class="profile__button dropdown__button">
			    ...
			    <div class="profile__content dropdown__content">
			      ...
			      <ul class="profile__list">
			        ...
			        <li class="profile__item">
			          <a href="javascript:;" v-on:click="logout()" class="profile__link">
			            Выйти
			          </a>
			        </li>
			      </ul>
			...
			axios.get("/api/v1/get_user_data").then(response => {
				...
				if(response.data.status) {
					...
					window.intercomSettings = {
  					...
  					(function(){var w=window;var ic=w.Intercom;if(typeof ic==="function"){ic('reattach_activator');ic('update',w.intercomSettings);}else{var d=document;var i=function(){i.c(arguments);};i.q=[];i.c=function(args){i.q.push(args);};w.Intercom=i;var l=function(){var s=d.createElement('script');s.type='text/javascript';s.async=true;s.src='https://widget.intercom.io/widget/jyugn4xp';var x=d.getElementsByTagName('script')[0];x.parentNode.insertBefore(s,x);};if(w.attachEvent){w.attachEvent('onload',l);}else{w.addEventListener('load',l,false);}}})();
				}
				try {
					setTimeout(() => {
						document.querySelector('[data-dropdown-target]').addEventListener('click', function() {
							...
							var dropping = function(event) {
								...
								if( ! firstClick ) {
									...
									document.removeEventListener('click', dropping)
									document.body.click()
								}
								firstClick = false
							}
