<template>
  <div>
    ...
    <div class="step-form-card-wrap">
      <button class="button submit-button" :disabled="disabled" type="button" @click="toPay()">
        {{ (b_text) ? b_text : "Оплатить" }}
        <template v-if="price > 0"> {{ price }} ₽</template>
      </button>
    </div>
  </div>
</template>
  ...
  methods: {
        ...
        this.updateData().then(response => {
          ...
          if (response.data.data.hasOwnProperty('Status') && response.data.data.Status != null) {
            if (response.data.data.Status == "Completed") {
              ...
              this.errors = "Оплата не прошла, пожалуйста повторите попытку позже"
    ...
    toPay() {
      ...
      if(response.data.data.hasOwnProperty('payed') && response.data.data.payed == 1){
          this.$emit('isPaying', true)
        }else if(this.method =='refund' || (response.data.data.hasOwnProperty('payment') && response.data.data.payment.hasOwnProperty('Status') && response.data.data.payment.Status != "Completed")) {
    ...
    charge() {
      ...
      if (this.user && this.tarrif && this.payment || this.user && this.payment && this.method == "refund") {
        let floatPrice = (this.method != "refund") ? this.tarrif.price.toFixed(2) : this.price.toFixed(2)
        let price = (this.method != "refund") ? this.tarrif.price : this.price
          ...
          cloudPayments: {
            "CustomerReceipt": {
              "Items": [//товарные позиции
                {
                  "label": (this.title) ? this.title : "Подписка на страховку", //наименование товара
